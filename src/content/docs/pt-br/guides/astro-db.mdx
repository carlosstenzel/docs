---
title: 'Astro DB'
description: Aprenda a usar Astro DB, um banco de dados SQL totalmente gerenciado, projetado exclusivamente para Astro.
...
Astro DB é um banco de dados SQL totalmente gerenciado, projetado exclusivamente para o Astro. Desenvolva localmente ou conecte-se a um banco de dados hospedado e gerenciado em nossa plataforma [Astro Studio](#astro-studio).

## Instalação

Adicionar Astro DB a um projeto Astro novo ou existente (requer `astro@4.5` ou maior) with the [`@astrojs/db` integration](/en/guides/integrations-guide/db/) (`v0.8.1` or later). Astro includes a built-in `astro add` command to automate this setup process for you.
